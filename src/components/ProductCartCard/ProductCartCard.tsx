...
import {addItem, CART_QUERY_KEY, cartQuery} from '../ProductCard/ProductCard.tsx';
...
const ProductCartCard = ({item}: {item: Product}) => {
  ...
  const {mutate} = useMutation({
    mutationFn: ({count, price}: {count: number, price: number}) => addItem(item.id, count, price),
  ...
  const handleRemove = () => {
    mutate({count: Math.max(count - 1, 0  ), price: storagePrice ? storagePrice / 100 : 0});
  };
  return (
      <View style={styles.container}>
        ...
        <View style={styles.counterWrapper}>
          <Counter count={count} onHandleMinus={handleRemove} onHandleAdd={handleAdd}/>
        </View>
