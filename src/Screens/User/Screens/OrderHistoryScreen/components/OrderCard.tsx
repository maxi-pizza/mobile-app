  ...
  return (
      <View style={styles.order}>
        <Pressable onPress={() => setIsOpen(!isOpen)} style={styles.cardHeader}>
          ...
          <Text style={status ? styles.yellowText : styles.redText}>{status ? 'Выполнен' : 'Отмена'}</Text>
          <Caret width="21" height="21" color="#727272" style={isOpen && styles.caretOpened}/>
        </Pressable>
        {isOpen && (
          <View style={styles.orderContent}>
            ...
            <View style={styles.orderStatus}>
              ...
              <Text style={[status ? styles.yellowText : styles.redText, styles.font]}>{status ? 'Выполнен' : 'Отмена'}</Text>
            </View>
            <View style={styles.horizontalBar}/>
